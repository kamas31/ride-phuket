...
## ADR-067: TikTok Pixel browser integration — no manual per-route PageView re-fire

**Status:** Accepted — **partially superseded by ADR-068** (this ADR's evidence about `ttq.page()`'s own behavior is unchanged and still accurate; its conclusion that no manual per-route call is needed turned out to only hold for `ttq.page()`'s internal signals, not for the Standard Event "PageView" that Test Events/Ads Manager actually read — see ADR-068)
**Date:** 2026-07-27

**What was the problem?**
User requested a browser-only TikTok Pixel integration (PageView, ViewContent on scooter fiches, Contact on real WhatsApp-to-shop clicks) fully isolated from the existing PostHog/Supabase `events` tracking (see ADR-066 and the header comment in `lib/posthog.ts`). The open question was how PageView should behave across Next.js App Router client-side navigations: the standard TikTok base snippet only fires `ttq.page()` once, on load — App Router navigations never reload the page, so naively following the snippet as-is would only ever record one PageView for an entire visit, however many routes the visitor browses.

**What was tried first and why it failed?**
The initial plan (drafted and presented to the user before any code was written) was to add a `usePathname()`-driven client component that called a manual `trackTikTokPageView()` (wrapping `ttq.page()`) on every route change, skipping only the very first mount to avoid double-counting that one. The user pushed back before implementation: TikTok Pixel has shipped automatic SPA route-change detection for some time, and the plan should not assume a manual re-fire is necessary without first verifying the real script's behavior on an actual Next.js App Router navigation.

Verified live, before writing any integration code:
1. Downloaded the actual pixel bootstrap payload for this project's real pixel ID (`curl https://analytics.tiktok.com/i18n/pixel/events.js?sdkid=D9JOV4BC77U820ARNJK0&lib=ttq`) and inspected the embedded remote-config JSON. It contains `"HistoryObserver":{"dynamic_web_pageview":true}` — a plugin flag enabled for this pixel, strongly suggesting native History-API-based SPA pageview detection.
2. Built a minimal Playwright probe (chromium, local static server) that loaded the **real** script with the **real** pixel ID, then simulated exactly what Next.js App Router does for client-side navigation — a bare `history.pushState()` call, nothing else — with **no** manual `ttq.page()` call after the initial load. Captured every `POST https://analytics.tiktok.com/api/v2/pixel` request and its JSON body.
   - Initial load (with one manual `ttq.page()` in the boilerplate, as TikTok's own docs snippet does): fired one `LandingPageView` (`trigger_source: "pixel_self"`, i.e. automatic) and one `Pageview` (our manual call) — two different event names, not a duplicate of the same metric.
   - Each subsequent `history.pushState()` — with zero manual calls — automatically fired exactly one `Pageview` event tagged `_inspection.tf: "hc"` (history-change) plus a separate `EngagedSession` signal. Confirmed across two consecutive pushState calls and a real `history.back()` (popstate): always exactly one `Pageview` per navigation, never zero, never two.
   - Control run: removed the manual `ttq.page()` from the boilerplate entirely. Result: **zero** `/api/v2/pixel` beacons of any kind fired, ever — not on load, not on any subsequent `pushState`. The SDK's page-tracking state must be "armed" by one manual `ttq.page()` call before its automatic HistoryObserver logic does anything.

**What decision was made?**
The base snippet is used exactly as TikTok's own official documentation specifies — `ttq.load(pixelId); ttq.page();` — mounted once via `components/analytics/TikTokPixel.tsx` (`next/script`, `strategy="afterInteractive"`, guarded by `NEXT_PUBLIC_TIKTOK_PIXEL_ID`) in the persistent root layout. No `usePathname()` hook, no route-change listener, and no other automatic re-fire of `trackTikTokPageView()` exists anywhere in this codebase. `trackTikTokPageView()` is still exported from `lib/analytics/tiktok.ts` as a safe, typed wrapper — available for a genuinely manual future call site — but it is intentionally not wired to anything today.

**Why this solution and not another?**
- The live evidence is unambiguous: TikTok's own `HistoryObserver` plugin already fires exactly one `Pageview` per `pushState`, which is precisely the mechanism Next.js App Router uses for client-side navigation. Adding our own re-fire on top would have produced exactly two `Pageview`-shaped events per navigation (one automatic, one manual) — the double-count the user explicitly asked to rule out first.
- Kept the manual `ttq.page()` in the initial boilerplate (rather than trying to rely on the automatic `LandingPageView` alone) because the control-run probe showed the automatic HistoryObserver mechanism does nothing at all — not even on the very first load — until one manual `page()` call has run. Omitting it would mean **zero** PageViews are ever recorded, not fewer.
- Chose a live, real-script, real-pixel-ID network probe over reading TikTok's minified/remote-config-driven SDK source or trusting official docs prose, because the actual enabled plugin set is pixel-specific server-side config (confirmed present in the downloaded payload, not a hardcoded default) and the only way to be certain of runtime behavior for *this* pixel was to observe it directly.
- The regression-safety test committed in `tests/e2e/tiktok-pixel.spec.ts` intentionally does **not** hit TikTok's live infrastructure (would be slow, flaky, and would send fake beacons to a real third party on every CI run). It intercepts the `events.js` request and replaces it with a tiny stub that records `ttq.page()`/`ttq.track()` calls, proving the invariant this codebase actually owns — our own bundle never manually re-fires PageView on navigation — without re-asserting TikTok's third-party SPA-detection behavior (out of our control, already verified above, and not meaningful to pin against a script that could change server-side at any time).

**What are the consequences or risks?**
- If TikTok ever changes this pixel's remote config to disable `HistoryObserver`/`dynamic_web_pageview`, PageView tracking on client-side route changes would silently stop working (no code-level symptom, since nothing in this codebase would be "broken" — the automatic mechanism this ADR relies on would simply no longer fire). Re-running the same live-probe methodology documented here is the way to re-verify if PageView counts in TikTok Ads Manager ever look wrong for this project.
- `trackTikTokViewContent`/`trackTikTokContact` are unaffected by any of this — they are always explicit, single-call-site events (`TrackView`'s mount effect; `WhatsAppButton`/`QuickContact`/`TikTokContactLink`'s `onClick` handlers), verified live against real scooter/shop pages with real data (see `docs/CHANGELOG_AI.md`).
- Validated via `npx tsc --noEmit` (clean, both `tsconfig.json` and `tsconfig.test.json`), `npm run lint` (89 problems: 14 errors/75 warnings — identical to the pre-existing baseline, zero new), `npm run test:unit` (52/52 passing, +9 new), and the new `tests/e2e/tiktok-pixel.spec.ts` (2/2 passing, run against two temporary local dev servers — one with `NEXT_PUBLIC_TIKTOK_PIXEL_ID` set, one without — to exercise both the presence and the silent-absence path).

---

## ADR-068: Correction — TikTok's Standard Event "PageView" requires an explicit `ttq.track('PageView')` per route; `ttq.page()`'s automatic SPA re-fire is a different, unrelated signal

**Status:** Accepted
**Date:** 2026-07-27

**What was the problem?**
After deploying ADR-067's integration to production, the user reported via real TikTok Test Events: `ViewContent` and `Contact` were arriving correctly, but **`PageView` was never received at all** — not on initial load, not on any navigation. ADR-067 had concluded, from a live probe, that TikTok's `HistoryObserver` plugin auto-fires "exactly one automatic Pageview per History API pushState" and that this was sufficient — no manual re-fire needed. That conclusion was real (the probe genuinely captured those network calls) but answered the wrong question: it never checked whether the event TikTok actually fired matched TikTok's own Standard Event schema.

**What was tried first and why it failed?**
Re-ran the exact same live-probe methodology from ADR-067 (real script, real pixel ID `D9JOV4BC77U820ARNJK0`, headless Chromium, captured every `POST https://analytics.tiktok.com/api/v2/pixel` body), but this time also called `ttq.track('PageView')` once on load alongside the existing `ttq.page()`, so both code paths' output could be compared side by side in the same run. The captured event names on initial load were: `LandingPageView` (automatic), `Pageview` — lowercase "v" — (from the manual `ttq.page()` call), and `PageView` — exact Standard Event casing — (from the manual `ttq.track('PageView')` call). Three different event names, from three different trigger paths.

The critical result: on two subsequent `history.pushState()` navigations with **no** further manual calls of any kind, the automatic re-fire produced `Pageview` (lowercase "v", tagged `_inspection.tf: "hc"`) and `EngagedSession` — the exact same internal-analytics signals ADR-067 had captured — but **never** another `PageView`. TikTok's `HistoryObserver` auto-re-fires its own internal telemetry event on route changes; it has no equivalent auto-behavior for the Standard Event pipeline that Test Events and Ads Manager conversion tooling actually read. ADR-067's probe was accurate about what it measured (`Pageview`, the internal signal) but that was never the same thing as the Standard Event `PageView` the integration was supposed to deliver — the two only differ by one letter's case, which is exactly what made the original conclusion look correct while being wrong for the metric that mattered.

**What decision was made?**
`trackTikTokPageView()` (`lib/analytics/tiktok.ts`) now calls `ttq.track('PageView')` instead of `ttq.page()`. `components/analytics/TikTokPixel.tsx` became a client component: the base snippet still calls `ttq.load(id); ttq.page();` unchanged (kept — see risks below) and now additionally calls `ttq.track('PageView');` once on load; a `usePathname()` effect with a `useRef`-guarded skip-on-first-mount then calls `trackTikTokPageView()` on every subsequent route change — the same single-source-of-truth pattern already used successfully for `ViewContent` (`TrackView`'s mount effect) and `Contact` (`onClick` handlers), which the user confirmed work correctly. Verified live, end-to-end, against TikTok's real collection endpoint with the actual fixed app code running: three navigations (initial load + two client-side route changes) produced exactly three `"event": "PageView"` beacons — one per navigation, in order, no duplicates, none missing.

**Why this solution and not another?**
- Did not try to coax the automatic `HistoryObserver` mechanism into firing the Standard Event (e.g. via some `ttq.page()` config option) — the live probe showed no such automatic path exists for `PageView` specifically; every `ttq.track('PageView')` call site in this integration is therefore explicit and code-owned, which is also consistent with how `ViewContent`/`Contact` already work in this codebase.
- Kept `ttq.page()` in the base snippet rather than removing it now that it isn't what drives PageView reporting: ADR-067's control-probe finding still holds unchanged — without at least one `ttq.page()` call, the SDK never sends any beacon at all, of any kind. Removing it risked silently breaking `ViewContent`/`Contact` too, which the user explicitly said must not be touched.
- The `usePathname()` skip-first-mount tracker is the same pattern the original (pre-ADR-067) plan proposed, before the user's initial push-back. That push-back was correct given what was tested at the time (`ttq.page()`'s own auto-behavior); it does not apply to `ttq.track('PageView')`, which has no automatic equivalent to defer to.
- `tests/e2e/tiktok-pixel.spec.ts` was rewritten around the corrected invariant: N navigations must produce exactly N `track:PageView` calls — catching both the original failure mode (missing PageView) and the failure mode the user explicitly warned against (a duplicate on the same navigation), rather than asserting a flat "stays at 1" count that was only ever true for the wrong event.

**What are the consequences or risks?**
- `ViewContent` and `Contact` are entirely unaffected by this change — same call sites, same payload shape (aside from the separate `content_type` fix below), confirmed by the user's own Test Events observation that both already worked correctly before this fix.
- `TikTokPixel.tsx` is now a client component (`'use client'`, `usePathname`) rather than a pure Server Component — a small bundle-size/hydration cost, unavoidable given `usePathname()` requires it; no other behavior change to the rest of the app.
- Re-affirms ADR-067's own stated risk: if TikTok's remote config for this pixel ever changes `ttq.page()`'s behavior such that it stops sending any beacon at all, this integration's "arming" assumption breaks silently. Unchanged by this ADR — re-run the same live-probe methodology to re-verify if needed.
- Fixed alongside, same session: `trackTikTokViewContent`'s `content_type` was `'scooter'`, which TikTok rejected ("Content type is invalid in your event. Allowed values are 'product' or 'product_group'.") — changed to `'product'`. `content_id`/`content_name` and the `Contact` payload shape are untouched. Verified live against the real endpoint on a real scooter page: `content_type: "product"` now sent, no other property changed.
- Validated via `npx tsc --noEmit` (clean, both configs), `npm run lint` (89 problems: 14 errors/75 warnings — identical baseline, zero new), `npm run test:unit` (52/52 passing), `tests/e2e/tiktok-pixel.spec.ts` (2/2 passing, run against two temporary local dev servers — with and without the pixel ID), and two live network probes against TikTok's real collection endpoint (one confirming 3-navigations-in/3-PageViews-out, one confirming the corrected `ViewContent` payload) — no code was considered done from theory alone.
