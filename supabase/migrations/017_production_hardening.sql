...
--
-- SAFE VERSION — fixed PostgreSQL array types
-- images / features / gallery / delivery_zones use text[] NOT jsonb
--
-- Strategy:
-- backfill NULLs FIRST
-- then add NOT NULL + DEFAULT constraints
-- then indexes
--
-- Safe for production.
...
-- 5. category
UPDATE public.scooters
SET category = 'automatic'
WHERE category IS NULL
   OR category NOT IN ('automatic', 'manual', 'electric');
...
-- 10. helmet_included
UPDATE public.scooters
SET helmet_included = false
WHERE helmet_included IS NULL;

-- 11. insurance_included
UPDATE public.scooters
SET insurance_included = false
WHERE insurance_included IS NULL;

-- 12. min_rental_days
UPDATE public.scooters
SET min_rental_days = 1
WHERE min_rental_days IS NULL
   OR min_rental_days < 1;

-- 13. passport flags
UPDATE public.scooters
SET passport_required = false
WHERE passport_required IS NULL;

UPDATE public.scooters
SET passport_copy_allowed = true
WHERE passport_copy_allowed IS NULL;

UPDATE public.scooters
SET is_premium_bike = false
WHERE is_premium_bike IS NULL;
...
-- delivery_zones (text[])
UPDATE public.shops
SET delivery_zones = ARRAY[]::text[]
WHERE delivery_zones IS NULL;

UPDATE public.shops
SET deposit_protected_member = false
WHERE deposit_protected_member IS NULL;
...
ALTER TABLE public.shops
  ...
  ALTER COLUMN delivery_zones SET NOT NULL,
  ALTER COLUMN delivery_zones SET DEFAULT ARRAY[]::text[],
