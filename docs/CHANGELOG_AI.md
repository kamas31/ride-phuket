...
## 2026-07-27 (session 25)

### Feature: TikTok Pixel browser integration (PageView, ViewContent, Contact)

**Why it was needed:** User requested a browser-only TikTok Pixel (`NEXT_PUBLIC_TIKTOK_PIXEL_ID`, real ID `D9JOV4BC77U820ARNJK0`) tracking PageView (initial load + client-side route changes, no duplicates), ViewContent on real scooter fiches, and Contact on real WhatsApp-to-shop clicks — fully isolated from the existing PostHog/business `events` tracking, no PII, no Events API in this mission, no npm dependency.

**What changed:**
- `lib/analytics/tiktok.ts` (new) — centralized module: `TIKTOK_PIXEL_ID`, `Window.ttq` typing (no `any`), `trackTikTokPageView()`, `trackTikTokViewContent({content_id, content_name})` (`content_type` hardcoded to `"scooter"`), `trackTikTokContact({scooter_id?, shop_id?, placement?})`. Every function no-ops on SSR, when the env var is unset, or when `window.ttq` isn't loaded, and never throws — same defensive pattern as `lib/posthog.ts`.
- `components/analytics/TikTokPixel.tsx` (new) — Server Component (no `'use client'` needed — it renders no hooks), mounts the official TikTok base snippet via `next/script` (`strategy="afterInteractive"`, single `id="tiktok-pixel"`), rendered only when the pixel ID is set. No `usePathname()` route-change hook — see ADR-067 for the live evidence that TikTok's own SDK already auto-fires PageView on client-side navigation, and that adding a manual re-fire would double it.
- `components/analytics/TikTokContactLink.tsx` (new) — thin `<a>` client wrapper (same pattern as the existing `CtaLink.tsx`), used only for the shop page's WhatsApp button, which previously had zero tracking of any kind.
- `app/layout.tsx` — mounts `<TikTokPixel />` as a sibling of `PostHogProvider` (not nested inside it), before `CapacitorProvider`.
- `components/analytics/TrackView.tsx` — added an optional `tiktokViewContent?: {content_id, content_name}` prop, fired inside the existing single-mount `useEffect` alongside the business/PostHog dispatch.
- `app/scooter/[id]/page.tsx` — passes `tiktokViewContent={{content_id: scooter.id, content_name: \`${scooter.brand} ${scooter.model}\`}}` to the existing `<TrackView>`; added `placement="scooter_mobile_cta"` / `"scooter_contact_card"` to its two `<WhatsAppButton>` usages.
- `app/scooter/[id]/WhatsAppButton.tsx` — added an optional `placement?: string` prop and a `trackTikTokContact({scooter_id, shop_id, placement})` call inside the existing synchronous `onClick`, alongside (not replacing) the existing `trackEvent`/`captureEvent` calls.
- `components/ride/StickyContactBar.tsx` — passes `placement="sticky_bar"` to its `<WhatsAppButton>`.
- `components/ride/QuickContact.tsx` — added `trackTikTokContact(...)` to its two existing `onClick` handlers (primary WhatsApp CTA, quick-question chips), placements `quick_contact_primary` / `quick_contact_chip`.
- `app/shop/[slug]/page.tsx` — replaced the previously untracked plain `<a href={waLink}>` "Continue on WhatsApp" button with `<TikTokContactLink>` (`placement="shop_page_cta"`, `shop_id` only — no `scooter_id`, since this button isn't scooter-specific). Visuals/behavior unchanged; this is the only call site where tracking of any kind is newly added rather than added alongside an existing call.
- `.env.example` — added `NEXT_PUBLIC_TIKTOK_PIXEL_ID=` (this file is gitignored in this repo — see "Problems encountered").
- `lib/analytics/tiktok.test.ts` (new) — 9 unit tests (`node --test`, picked up automatically by the existing `lib/**/*.test.ts` glob in `npm run test:unit`): SSR no-op, no-op without pixel ID, no-op without `window.ttq`, correct ViewContent/Contact payload shape, no PII fields ever present, exceptions from `ttq.page()`/`ttq.track()` swallowed.
- `tests/e2e/tiktok-pixel.spec.ts` (new) — Playwright regression test. Intercepts the `events.js` request with a small local stub (never hits TikTok's live infrastructure) that replays the queued bootstrap calls, then asserts `ttq.page()` fires exactly once on load and stays at exactly one call after two real client-side navigations (via actual `<Link>` clicks) — this is the "our own code never re-fires PageView" guarantee; also asserts the script is entirely absent when the pixel ID is unset.

**Problems encountered:**
- The originally-planned design (before implementation) included a `usePathname()`-driven manual PageView re-fire on every route change. The user flagged, before any code was written, that TikTok Pixel has native SPA PageView detection and asked for live verification before assuming a manual tracker was needed. A real network probe (downloaded the actual `events.js` for the real pixel ID, ran it in a headless browser against a simulated `history.pushState()` navigation) confirmed TikTok's `HistoryObserver` plugin already auto-fires exactly one `Pageview` per route change — a manual re-fire would have double-counted. Full methodology and captured evidence in ADR-067.
- The same probe also showed that without at least one manual `ttq.page()` call, the pixel never sends **any** beacon at all, ever (not even the automatic ones) — so the standard `ttq.load(id); ttq.page();` boilerplate is kept exactly as TikTok's own docs specify, on load only.
- The first version of `tests/e2e/tiktok-pixel.spec.ts` had a race condition: it checked `#tiktok-pixel`'s presence immediately after `gotoAndWait` (which only waits for `domcontentloaded`), but `next/script`'s `afterInteractive` strategy injects the script client-side, after hydration — later than `domcontentloaded`. This produced a false "script absent" result even when the pixel was correctly configured. Fixed by waiting on an explicit condition (`page.waitForSelector('#tiktok-pixel', {state: 'attached'})` / `page.waitForFunction(...)` on the recorded call count) instead of a fixed-position check, for every assertion in the file.
- `.env.example` is covered by this repo's `.env*` `.gitignore` pattern and isn't tracked in git history at all (confirmed via `git log -- .env.example` and `git show HEAD:.env.example`, both empty/failing). The `NEXT_PUBLIC_TIKTOK_PIXEL_ID=` line was still added to the file on disk as requested, but it won't appear in `git status`/diffs or any commit — flagged to the user directly rather than silently changing `.gitignore`.
- `ttq.track('Contact', params)` failed `tsc --noEmit` (`TikTokContactParams` lacks an index signature, so it isn't structurally assignable to `TtqObject.track`'s `Record<string, unknown>` parameter). Fixed by spreading into a fresh object literal at the call site (`ttq.track('Contact', { ...params })`) rather than adding an index signature to the public `TikTokContactParams` interface, which would have silently permitted arbitrary extra keys (undermining the "no PII, metadata only" contract on that type).

**How they were solved:**
- Live verification methodology: `curl`'d the real `events.js` payload for pixel ID `D9JOV4BC77U820ARNJK0`, found `"HistoryObserver":{"dynamic_web_pageview":true}` in its embedded remote config; then built a disposable Playwright probe (local static HTML + the real script, real pixel ID) that captured every `POST https://analytics.tiktok.com/api/v2/pixel` request/body across an initial load, two `history.pushState()` calls, and a real `history.back()` — confirming exactly one `Pageview` event (tagged `_inspection.tf: "hc"` for the history-change-triggered ones) per navigation, zero duplicates, and zero events at all when the manual `ttq.page()` is omitted from the boilerplate. Probe scripts were scratch files, deleted after use — not committed.
- End-to-end smoke-verified against a real running dev server (temporary instance, `NEXT_PUBLIC_TIKTOK_PIXEL_ID` set only for that process, `.env.local` untouched) with real Supabase data: a real scooter page fired exactly one `ViewContent` with the correct `content_id`/`content_name`/`content_type: "scooter"`; clicking its WhatsApp button fired exactly one `Contact` with `scooter_id`/`shop_id`/`placement`, before the WhatsApp tab opened; a real shop page's "Continue on WhatsApp" button fired exactly one `Contact` with `shop_id`/`placement` only (no `scooter_id`, correctly). No PII in any payload.
- `npx tsc --noEmit` (app + `tsconfig.test.json`) clean; `npm run lint` → 89 problems (14 errors / 75 warnings), identical to the pre-existing baseline (confirmed by running lint before any change), zero new issues; `npm run test:unit` → 52/52 passing (43 pre-existing + 9 new); `tests/e2e/tiktok-pixel.spec.ts` → 2/2 passing, run against two temporary local dev servers (one with the pixel ID set, one without) to genuinely exercise both branches rather than assuming.

### Fix: TikTok Pixel — PageView never received in Test Events; ViewContent content_type rejected (ADR-068)

**Why it was needed:** After the integration above went live, the user reported via real TikTok Test Events that `ViewContent`/`Contact` arrived correctly but `PageView` never did at all, and TikTok flagged `ViewContent`'s `content_type: "scooter"` as invalid ("Allowed values are 'product' or 'product_group'.").

**What changed:**
- `lib/analytics/tiktok.ts` — `trackTikTokPageView()` now calls `ttq.track('PageView')` instead of `ttq.page()`. `trackTikTokViewContent()`'s `content_type` changed from `'scooter'` to `'product'` — `content_id`/`content_name` and the `Contact` payload shape untouched.
- `components/analytics/TikTokPixel.tsx` — converted to a client component (`'use client'`). The base snippet keeps its existing `ttq.load(id); ttq.page();` unchanged and now additionally calls `ttq.track('PageView');` once on load. A new `usePathname()` effect, guarded by a `useRef` that skips its own first mount, calls `trackTikTokPageView()` on every subsequent client-side route change — same single-source-of-truth pattern already proven correct for `ViewContent`/`Contact`.
- `lib/analytics/tiktok.test.ts` — updated the `content_type` assertion to `'product'`; updated `trackTikTokPageView`'s tests to assert a `ttq.track('PageView')` call shape instead of `ttq.page()`.
- `tests/e2e/tiktok-pixel.spec.ts` — rewritten around the corrected invariant: N navigations (initial load + N-1 client-side route changes) must produce exactly N `track:PageView` calls, asserted incrementally after each navigation — catches both the original bug (PageView missing entirely) and a regression to a duplicate-per-navigation, rather than the previous (now-inaccurate) "stays flat at 1 forever" assertion.

**Problems encountered:**
- ADR-067's original live probe was real and accurate but answered the wrong question: it confirmed `ttq.page()`'s automatic `HistoryObserver` re-fire produces one event per `history.pushState()` navigation, and concluded no manual re-fire was needed — without checking whether that auto-fired event was actually the Standard Event "PageView" TikTok's Test Events/Ads Manager read. It wasn't: the auto-fired event is named `Pageview` (lowercase "v", an internal analytics signal), a one-letter-case difference from the Standard Event `PageView` that made the original conclusion look right while missing the metric that actually mattered.

**How they were solved:**
- Re-ran the exact ADR-067 live-probe methodology (real script, real pixel ID, headless Chromium, captured every `POST https://analytics.tiktok.com/api/v2/pixel` body) but this time called both `ttq.page()` and `ttq.track('PageView')` on load, side by side, to compare their actual event names. Confirmed: manual `ttq.page()` → `"Pageview"`; manual `ttq.track('PageView')` → `"PageView"` (exact Standard Event); two subsequent `history.pushState()` navigations with zero manual calls only ever auto-re-fired `Pageview`/`EngagedSession` — never another `PageView`. This is the root cause, confirmed from real captured payloads, not documentation or assumption.
- Verified the fix end-to-end against TikTok's real collection endpoint with the actual fixed app code running (temporary dev server, real pixel ID, `.env.local` untouched): 3 navigations (load + 2 client-side route changes) produced exactly 3 `"event": "PageView"` beacons, one per navigation, zero duplicates, none missing. Separately verified a real scooter page now sends `content_type: "product"` with `content_id`/`content_name` unchanged.
- `npx tsc --noEmit` (both configs) clean; `npm run lint` → 89 problems (14 errors / 75 warnings), identical baseline, zero new; `npm run test:unit` → 52/52 passing; `tests/e2e/tiktok-pixel.spec.ts` → 2/2 passing against two temporary dev servers (with and without the pixel ID).
